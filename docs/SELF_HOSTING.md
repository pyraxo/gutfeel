...
only bounded local logs. The tunnel runs with fatal-only logging because the
browser's WebSocket URL contains a scoped lobby credential.

Verify the public route after every update:
...
The health response must report `{"status":"ok"}`, and the homepage must
include HSTS, `Referrer-Policy`, `Permissions-Policy`, and `X-Frame-Options`.
